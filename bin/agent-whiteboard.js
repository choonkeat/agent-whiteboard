...
if (!platform || !arch) {
  console.error(
    `Unsupported platform: ${process.platform}-${process.arch}\n` +
      `agent-whiteboard supports: linux-x64, linux-arm64, darwin-x64, darwin-arm64, win32-x64, win32-arm64`
  );
  process.exit(1);
}
...
if (result.error) {
  console.error(`Failed to start agent-whiteboard: ${result.error.message}`);
  process.exit(1);
}
